...
from models import PlayerModel, UserModel, UpdateUserModel, CreateUserModel
...
async def create_user(request: Request, user: CreateUserModel = Body(...)):
    ...
    if (existing_user := await request.app.mongodb["users"].find_one({"user_id": user['user_id']})) is not None:
...
@router.get("/user/{user_id}", response_description="Get the player score", response_model=UserModel)
async def get_player_data(request:Request, user_id:str):

    if (existing_user := await request.app.mongodb["users"].find_one({"user_id":user_id})) is not None:
        return existing_user
...
@router.put("/{user_id}", response_description="Update a player score", response_model=UserModel)
async def update_score(
    ...
    if len(user) >= 1:
        ...
        if update_result.modified_count == 1:
            if (
                updated_user := await request.app.mongodb["users"].find_one({"user_id": user_id})
            ) is not None:
                return updated_user
...
async def list_all_players(request: Request, num: int = 10):
    ...
    results = [
        format_question(dict(PlayerModel(**raw_player))) async for raw_player in full_query
        ]
